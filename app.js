...
function allItems(){
  ...
  const offers=state.catalog.offers.map(item=>({...item,kind:'offer',features:[item.description],specs:[['Product type',item.category],['Commerce','Prelaunch']],included:['Lux ecosystem access when commissioned']}))
  return products.concat(offers)
}
...
function renderChrome(){
  ...
  if(chrome)chrome.innerHTML='<div class="announcement"><span>PUBLIC STORE PREVIEW · COMMERCE ACTIVATES AFTER FINAL PRICING</span><span>NEW · LUX AGENT VIEWER</span><span>NEW PUBLIC KITS REQUIRE LUX VERIFY PASS</span></div>'+
  '<header class="store-header"><a class="store-brand" href="./index.html"><strong>LUX</strong><span>STORE</span><i></i></a>'+
...
function openBag(){renderBag();$('#bag-backdrop').hidden=false;$('#bag-drawer').classList.add('open');$('#bag-drawer').setAttribute('aria-hidden','false')}
function closeBag(){if($('#bag-backdrop'))$('#bag-backdrop').hidden=true;$('#bag-drawer')?.classList.remove('open');$('#bag-drawer')?.setAttribute('aria-hidden','true')}
function renderBag(){
...
function itemVisual(item,size){
  if(item.images&&item.images.length)return '<img class="item-image '+(size||'')+'" src="./'+esc(item.images[0])+'" alt="'+esc(item.name)+'">'
...
fetch('./data/catalog.json',{cache:'no-store'}).then(response=>{if(!response.ok)throw new Error('Catalog '+response.status);return response.json()}).then(catalog=>{
  state.catalog=catalog;renderBag()
